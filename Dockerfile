...
ENV CGO_ENABLED=0 GOOS=linux GOARCH=arm64 DEBIAN_FRONTEND=noninteractive
RUN apt-get update && \
    apt-get install --no-install-recommends -y curl git-core make && \
    apt-get clean all

RUN curl -LO https://dl.google.com/go/go${GO_VERSION}.linux-arm64.tar.gz && \
    SUM=$(sha256sum go${GO_VERSION}.linux-arm64.tar.gz | awk '{print $1}') && \
...
RUN CGO_ENABLED=0 GOOS=linux GOARCH=arm64 GO111MODULE=on go build -ldflags "-s -w" -a -o manager main.go
